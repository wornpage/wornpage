...
The package expects the Wornpage `--worn-*` color and typography tokens. Override `--worn-binary-boundary` on a control only when a host theme has independently verified a 3:1 state boundary. Keyboard outlines use `--worn-binary-focus`; the default follows `currentColor` while unchecked and `--worn-accent-text` while checked.
